...
public class ClubDeportivo {
    ...
    public void registrarReserva(Reserva reserva) {
        ...
        if (reserva.getNumeroJugadores() > reserva.getInstalacion().getCapacidadMaxima()) {
            throw new IllegalArgumentException("La cantidad de jugadores supera la capacidad");
        }
        for (Reserva existente : reservas) {
            boolean mismaInstalacion = existente.getInstalacion() == reserva.getInstalacion();
            boolean mismaFecha = existente.getFecha().equals(reserva.getFecha());
            boolean seSuperponen = reserva.getHoraInicio().isBefore(existente.calcularHoraFin())
                    && existente.getHoraInicio().isBefore(reserva.calcularHoraFin());
            if (mismaInstalacion && mismaFecha && seSuperponen) {
    ...
    public ArrayList<ExistenciaInventario> getExistencias() {
        return new ArrayList<ExistenciaInventario>(existencias);
    }
    ...
    public ArrayList<SesionEntrenamiento> getSesiones() {
        return new ArrayList<SesionEntrenamiento>(sesiones);
    }
    ...
    public ExistenciaInventario buscarExistencia(
            Producto producto, UbicacionInventario ubicacion) {
    ...
    public void reabastecerProducto(
            Producto producto, UbicacionInventario ubicacion, int cantidad) {
        ...
        if (existencia == null) {
            existencia = new ExistenciaInventario(producto, ubicacion, cantidad);
            existencias.add(existencia);
        } else {
    ...
    public void trasladarInventario(
        ...
        if (origen == destino) {
            throw new IllegalArgumentException("Las ubicaciones deben ser diferentes");
        }
        if (cantidad <= 0) {
            throw new IllegalArgumentException("La cantidad debe ser mayor que cero");
        }
        ExistenciaInventario existenciaOrigen = buscarExistencia(producto, origen);
        if (existenciaOrigen == null || existenciaOrigen.getCantidad() < cantidad) {
        ...
        if (existenciaDestino == null) {
            existenciaDestino = new ExistenciaInventario(producto, destino, cantidad);
    ...
    public void procesarVenta(Venta venta, UbicacionInventario ubicacion) {
        ...
        for (Map.Entry<Producto, Integer> entrada : cantidades.entrySet()) {
            ExistenciaInventario existencia = buscarExistencia(
                    entrada.getKey(), ubicacion);
            existencia.retirarUnidades(entrada.getValue());
        }
        if (venta.getComprador() instanceof Socio) {
            ...
            empleado.agregarPuntos(venta.getPuntosGenerados());
        }
        ventas.add(venta);
    }

    public void validarIngresoProducto(
            Producto producto, Instalacion instalacion) {
        ...
        if (producto instanceof Bebida) {
            ...
            if (bebida.isCaliente()
                    && (instalacion instanceof CanchaPadel
    ...
    public boolean cumpleCoberturaMinima(Turno turno) {
        ...
        return cantidadEntrenadores >= 2 && cantidadFisioterapeutas >= 1;
    }
}
